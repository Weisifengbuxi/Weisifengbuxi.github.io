const CountdownTimer = (() => {
    ...
    const calculators = {
        day: () => {
            ...
            return {
                remaining: 24 - hours - 1 ,
                percentage: ((hours + 1)/ 24) * 100
            };
        },
        week: () => {
        ...
        month: () => {
            ...
            const today = now.getDate(); // 获取今天的日期（1-31）
            // 修正：剩余天数 = 总天数 - 今天的日期
            return {
                remaining: total - today,
                percentage: (today / total) * 100
            };
        },
        year: () => {
            const now = new Date();
    ...
    // 考虑可能的毫秒级误差，使用Math.round而不是Math.floor
    const passed = Math.round(diffTime / (1000 * 60 * 60 * 24));

    // 确保不会出现负数或超过全年天数的情况
    const safePassed = Math.max(0, Math.min(passed, total));

    return {
        remaining: total - safePassed,
        percentage: (safePassed / total) * 100
    };
        }
    };
    ...
    function updateCountdown() {
        ...
        const daysDiff = Math.ceil((target - tomorrowStart) / 86400000);
        daysUntil.textContent = Math.max(daysDiff, 0);
